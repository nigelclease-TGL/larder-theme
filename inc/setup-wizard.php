...
function nkt_render_setup_page() {
	...
	<section class="nkt-setup-card"><h2><?php esc_html_e( '2. Brand and navigation', 'larder' ); ?></h2><ul class="nkt-status-list"><?php nkt_setup_status_item( $hero_ready, __( 'Homepage hero image selected', 'larder' ) ); nkt_setup_status_item( $portrait_ready, __( 'Small About Nigel portrait selected', 'larder' ) ); nkt_setup_status_item( true, __( 'Branded favicon package included', 'larder' ), __( 'The bundled NKT icon covers browser tabs, Apple devices and Android shortcuts. A WordPress Site Icon may still override it.', 'larder' ) ); nkt_setup_status_item( $primary_menu, __( 'Primary menu assigned', 'larder' ) ); nkt_setup_status_item( $footer_menu, __( 'Footer menu assigned', 'larder' ) ); ?></ul><p><a class="button" href="<?php echo esc_url( admin_url( 'customize.php' ) ); ?>"><?php esc_html_e( 'Open Customizer', 'larder' ); ?></a> <a class="button" href="<?php echo esc_url( admin_url( 'nav-menus.php' ) ); ?>"><?php esc_html_e( 'Manage menus', 'larder' ); ?></a></p></section>
