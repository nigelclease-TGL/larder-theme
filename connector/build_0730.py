...
def build_openapi() -> Path:
    schema = json.loads(build_0729_openapi().read_text(encoding='utf-8').replace('0.7.29', '0.7.30'))
    schema['info'].update({
        'title': "Nigel's Kitchen Table GPT Connector - Compact Complete 0.7.30",
        'version': '0.7.30-schema.1',
        'description': (
            'Complete compact schema for connector 0.7.30. Retains all 27 existing actions and adds one standalone '
            'read-only reusable-block object evidence action. It does not alter protected lifecycle acceptance or write WordPress data.'
        ),
    })
    schema['paths']['/protected-article-revisions/legacy-reconciliation']['get']['description'] = (
        'Read-only paginated inventory of drafts with connector metadata signals. It does not infer ownership from titles, '
        'authors, slugs, dates or content similarity. Returns classifications, evidence hashes, failures and successor IDs.'
    )
    schema['paths']['/protected-article-revisions/legacy-reconciliation/record']['post']['description'] = (
        'Requires exact obsolete and successor draft pairs plus current evidence and lifecycle guards. Refuses the full batch '
        'before writing on any mismatch. A non-dry-run writes metadata only and restores it if verification fails.'
    )
    schema['paths']['/reusable-block-evidence'] = {'get': {
        'operationId': 'inspectReusableBlockEvidence',
        'summary': 'Inspect one reusable-block object and its literal WordPress state',
        'description': (
            'Read-only canonical wp_block lookup by numeric ID. Returns literal type, status, dates, content hashes, direct '
            'block references and exact missing, wrong-type or access classification without changing WordPress.'
        ),
        'parameters': [
    ...
    if len(operations) != 28 or len(set(operations)) != 28 or operations.count('inspectReusableBlockEvidence') != 1:
...
def build_release() -> tuple[Path, Path]:
    ...
    with zipfile.ZipFile(zip_path, 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        for source_path, target in files:
            info = zipfile.ZipInfo('nkt-gpt-connector-upgrader-0.7.30/' + target, date_time=(2026, 8, 2, 0, 0, 0))
