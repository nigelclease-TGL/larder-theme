...
SRC = ROOT / 'artifacts' / 'generated' / 'protected-lifecycle-0.7.27.php'
SCHEMA = ROOT / 'artifacts' / 'generated' / 'openapi-0.7.27.json'
UPDATER = ROOT / 'artifacts' / 'generated' / 'nkt-gpt-connector-upgrader-0.7.27.php'
...
check('connector source targets 0.7.27', "const NKT_GPT_PAR_0723_VERSION         = '0.7.27';" in text)
check('connector runtime guard targets 0.7.27', "'0.7.27' !== NKT_GPT_CONNECTOR_VERSION" in text)
...
check('canonical label helper exists', 'nkt_gpt_par_0723_canonical_nutrient_label' in text)
check('canonical helper strips one terminal colon', "preg_replace( '/\\s*:\\s*$/u', '', $text, 1" in text)
...
check('adjacent records canonicalized', '$record_label = nkt_gpt_par_0723_canonical_nutrient_label( $record );' in text)
...
check('status accepts stored baselines through 0.7.27', "'0.7.23', '0.7.24', '0.7.25', '0.7.26', '0.7.27'" in text)
ops = []
...
check('schema public version is 0.7.27', schema['info']['version'].startswith('0.7.27'))
check('schema contains no earlier public version', not any(v in schema_text for v in ['0.7.23','0.7.24','0.7.25','0.7.26']))
...
check('updater source version exact', "NKT_GPT_UPGRADER_0727_SOURCE_VERSION = '0.7.26'" in updater)
check('updater target version exact', "NKT_GPT_UPGRADER_0727_TARGET_VERSION = '0.7.27'" in updater)
check('updater uses generated lifecycle', 'protected-lifecycle-0.7.27.php' in updater)
check('updater identifies source lifecycle', 'protected-lifecycle-0.7.26.php' in updater)
...
check('updater does not require exact comment marker', 'exact expected 0.7.26 source markers' not in updater)
check('updater replaces source constant by regex once', 'preg_replace( $source_constant_pattern' in updater)
check('updater replaces source loader by regex once', '$loader_pattern' in updater and 'protected-lifecycle-0\\.7\\.26' in updater)
...
check('updater names previous updater exactly', 'NKT GPT Connector 0.7.26 Structured Nutrient Evidence Upgrader' in updater)
check('updater contains no stale escaped 0.7.24', '0\\.7\\.24' not in updater)

runtime = subprocess.run(['php', str(ROOT / 'tests' / 'runtime_harness_0727.php')], capture_output=True, text=True)
