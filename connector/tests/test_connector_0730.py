...
check('draft wp_block literal status returned', "'literal_wordpress_status'" in text and "post_status" in text)
check('private wp_block literal status returned', "(string) $post->post_status" in text)
check('trashed wp_block can be inspected by raw lookup', "get_post( $block_id, OBJECT, 'raw' )" in text and "'missing_or_deleted'" in text)
...
check('access failure classification exists', "'exists_inaccessible'" in text and 'read_post_capability_denied' in text)
check('nested direct reference scan exists', 'nkt_gpt_par_0730_direct_block_references' in text and 'direct_core_block_references' in text)
...
check('response declares read only', "'read_only'" in text and "'writes_attempted'" in text)

callback_start = text.index('function nkt_gpt_par_0730_inspect_reusable_block')
callback_end = text.index('/** Register the standalone 0.7.30')
...
for forbidden in ['wp_update_post', 'wp_insert_post', 'wp_delete_post', 'wp_trash_post', 'update_post_meta', 'delete_post_meta', 'add_post_meta']:
    check('callback contains no mutation ' + forbidden, forbidden not in callback)
...
check('schema exposes 28 actions', len(ops) == 28 and len(set(ops)) == 28)
